...
export async function CellsNetworksPaginateAction(page = 1, search = ""): Promise<IResponseObject<IPaginatedData<CellNetworkEntity>>> {
  try {
    ...
    const data = await fetchApi(FetchCtx.ERP, `/cells-networks?${params.toString()}`, {
      cache: "no-store"
    });
